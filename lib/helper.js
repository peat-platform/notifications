...
var cloudlet = {'aaaa': {'me': {'age': 23, 'name': 'Castro Hebert', 'gender': 'male',
   'company': 'Bulljuice', 'email': 'castrohebert@bulljuice.com', 'phone': '+1 (836) 512-3958',
   'address': '333 Bergen Court, Romeville, Connecticut, 672',
   'about': 'a sint elit Lorem. a sunt eu labore ullamco qui.\r\n',
   'friends': [
   {'id': 0, 'name': 'Maricela Rogers'},
   {'id': 1, 'name': 'Bridgett James'},
   {'id': 2, 'name': 'Hopper Stone'}
], 'subscription'                    : [
   {'facebook': [
      {'email': {'email': false, 'sms': false, 'ping': true, 'post': false}},
      {'address': {'email': false, 'sms': true, 'ping': true, 'post': false}},
      {'password': {'email': true, 'sms': true, 'ping': true, 'post': false}}
   ]},
   {'twitter': [
      {'email': {'email': false, 'sms': false, 'ping': true, 'post': false}},
      {'chat': {'email': true, 'sms': true, 'ping': true, 'post': true}},
      {'password': {'email': false, 'sms': true, 'ping': true, 'post': true}}
   ]},
   {'myspace': [
      {'chant': {'email': false, 'sms': true, 'ping': true, 'post': true}},
      {'password': {'email': true, 'sms': true, 'ping': true, 'post': false}},
      {'friend': {'email': false, 'sms': false, 'ping': false, 'post': true}}
   ]}
]}}, 'bbbb'           : {'me': {'age': 39, 'name': 'Dollie Shields', 'gender': 'female',
   'company': 'Paragonia', 'email': 'dollieshields@paragonia.com', 'phone': '+1 (814) 506-2738',
   'address': '261 Chester Street, Saticoy, Alabama, 3090',
   'about': 'Reprehenderit nisi occaecat iut aborum.\r\n',
   'friends': [
   {'id': 0, 'name': 'Robert Le'},
   {'id': 1, 'name': 'Dorthy Barnett'},
   {'id': 2, 'name': 'Mullins Franks'}
], 'subscription'                    : [
   {'facebook': [
      {'friend': {'email': false, 'sms': false, 'ping': false, 'post': true}},
      {'chat': {'email': true, 'sms': false, 'ping': true, 'post': true}},
      {'password': {'email': true, 'sms': true, 'ping': false, 'post': true}}
   ]},
   {'tumblr': [
      {'chat': {'email': true, 'sms': false, 'ping': false, 'post': false}},
      {'password': {'email': true, 'sms': false, 'ping': false, 'post': true}},
      {'email': {'email': true, 'sms': false, 'ping': false, 'post': false}}
   ]},
   {'myspace': [
      {'email': {'email': false, 'sms': true, 'ping': false, 'post': false}},
      {'chat': {'email': true, 'sms': true, 'ping': false, 'post': false}},
      {'password': {'email': true, 'sms': true, 'ping': false, 'post': true}}
   ]}
]}}, 'cccc' : {'me': {'age': 20, 'name': 'Elsa Kinney', 'gender': 'female', 'company': 'Katakana',
   'email': 'elsakinney@katakana.com', 'phone': '+1 (897) 557-3176',
   'address': '154 Midwood Street, Kennedyville, Minnesota, 4912',
   'about': 'Irure culpaaliquip aute et non duis sint.\r\n',
   'friends': [
   {'id': 0, 'name': 'Burton Patton'},
   {'id': 1, 'name': 'Bass Craft'},
   {'id': 2, 'name': 'Wallace Hughes'}
], 'subscription'                    : [
   {'tumblr': [
      {'email': {'email': true, 'sms': false, 'ping': true, 'post': true}},
      {'chat': {'email': false, 'sms': true, 'ping': false, 'post': true}},
      {'friend': {'email': true, 'sms': false, 'ping': false, 'post': false}}
   ]},
   {'myspace': [
      {'email': {'email': true, 'sms': true, 'ping': false, 'post': true}},
      {'chat': {'email': false, 'sms': false, 'ping': false, 'post': false}},
      {'friend': {'email': true, 'sms': true, 'ping': true, 'post': false}}
   ]},
   {'twitter': [
      {'email': {'email': false, 'sms': true, 'ping': false, 'post': true}},
      {'chat': {'email': false, 'sms': false, 'ping': true, 'post': true}},
      {'friend': {'email': false, 'sms': true, 'ping': false, 'post': false}}
   ]}
]}}}
...
var getData = function (msg, callback) {
   ...
   if (cloudlet[msg.cloudlet] != null && cloudlet[msg.cloudlet]['me']['subscription']) {
      var subscription = cloudlet[msg.cloudlet]['me']['subscription']
<<<<<<< HEAD
