...
var init = function (config) {
   ...
   db = new couchbase.Connection({host: 'localhost:8091', bucket: 'objects', connectionTimeout:60000}, function(err){
      if (err) {
         console.log('Connection Error', err);
         loglet.error(err);
      } else {
         console.log('Notification: Connected to "OPENi objects" Bucket');
      }
   });
};
