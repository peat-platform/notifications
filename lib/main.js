...
var notifApi = function (config) {
   ...
   zmq.receiver(config.mongrel_handler.subsource, config.mongrel_handler.subsink, function (msg) {
      ...
      if ( undefined === msg.headers.authorization ){
         sendToMongrel2.send(msg.uuid, msg.connId, zmq.status.BAD_REQUEST_400, zmq.standard_headers.json, {"error":"Missing Auth token: " });
         return
      }
      ...
      jwt.verify(tokenB64, config.trusted_security_framework_public_key, function(err, token) {
         ...
         if (undefined !== err && null !== err){
            sendToMongrel2.send(msg.uuid, msg.connId, zmq.status.BAD_REQUEST_400, zmq.standard_headers.json, {"error":"Invalid token: " + err });
         }
         else {
            if (token){
               msg.token = token;
            ...
            subsHelp.evaluateMessage(msg, function (err, results, httpStatusCode) {
               ...
               subsToMongrel2.send(msg.uuid, msg.connId, httpStatusCode, zmq.standard_headers.json, resp)
            });
         }
      });
